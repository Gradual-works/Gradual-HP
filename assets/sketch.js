...
let bouncerSize; // half the bouncer size
...
// images
let logoMargin = 20; // margin around the logo
let showBG = true; // show the background image
let bgImg0, bgImg1, bgImg2, bgImg3; // images for each layer

// mobile: mouse animation
// array for our mouse points
let mousePoints = [];

// cursor animation
let speed = 0.07;
let currentPoint = 0; // the point in our array that we currently move towards

/* - - Preload - - */
function preload() {
  bgImg0 = loadImage("assets/Frame_96.png"); // layer 0 (background image)
  bgImg1 = loadImage("assets/gradual.png"); // layer 1 (logo)
...
function draw() {
  ...
  if (isTouch) {
    ...
    // target positions
    let targetX = mousePoints[currentPoint][0];
    let targetY = mousePoints[currentPoint][1];

    mx = lerp(mx, targetX, speed);
    my = lerp(my, targetY, speed);

    // when close to target, move to the next one
    if (dist(mx, my, targetX, targetY) < 1) {
      currentPoint++;
      if (currentPoint >= mousePoints.length) {
...
// function: check if device is touch
...
function windowResized() {
  ...
  // recalculate cursor animation points
  mousePoints = [
    [width * 0.2, height * 0.9],
    [width * 0.8, height * 0.4],
    [width * 0.6, height * 0.9],
